...
run_functional_test_groups() {
  command="yarn test:playwright:civil-wa-pr:ci --grep "
...
run_failed_functional_tests() {
  ...
  if [ "$ENVIRONMENT" = "aat" ]; then
    if ! yarn test:playwright:civil-wa-master:ci --last-failed; then
      run_playwright_teardown
      exit 1
    fi
  else
    if ! yarn test:playwright:civil-wa-pr:ci --last-failed; then
      run_playwright_teardown
      exit 1
    fi
  fi
...
run_functional_tests() {
  ...
  if [ "$ENVIRONMENT" = "aat" ]; then
    if ! yarn test:playwright:civil-wa-master:ci; then
      run_playwright_teardown
      exit 1
    fi
  elif [ -z "$PLAYWRIGHT_PR_FT_GROUPS" ]; then
    if ! yarn test:playwright:civil-wa-pr:ci; then
      run_playwright_teardown
      exit 1
    fi
  else
